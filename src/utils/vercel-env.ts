...
const removeEnv = async (deploymentEnv: DeploymentEnv, envMap: EnvMap) => {
  ...
  for (const varName in envMap) {
    stdoutArr.push(
      limit(() => exec( `vercel env rm ${varName} ${deploymentEnv === "all" ? "" : deploymentEnv} -y` ).then(printStdout))
    )
  }
...
const addEnv = async (deploymentEnv: DeploymentEnv, envMap: EnvMap) => {
  ...
  for (const varName in envMap) {
    ...
    stdoutArr.push(
      limit(() => exec( `printf %s "${varValue}" | vercel env add ${varType} ${varName} ${deploymentEnv === "all" ? "" : deploymentEnv}` ).then(printStdout))
    )
  }
